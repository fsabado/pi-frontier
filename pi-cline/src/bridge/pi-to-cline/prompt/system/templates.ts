...
export type ClinePromptFamily = "generic" | "trinity";

const genericPromptTemplate = `You are Cline, a highly skilled software engineer with extensive knowledge in many programming languages, frameworks, design patterns, and best practices.
...
Parameters:
- path: (required) The path of a directory (not a file) relative to the current working directory {{cwd}}. Lists definitions across all source files in that directory. To inspect a single file, use read_file instead.
- task_progress: (optional) A checklist showing task progress after this tool use is completed. The task_progress parameter must be included as a separate parameter inside of the parent tool call, it must be separate from other parameters such as content, arguments, etc. (See 'UPDATING TASK PROGRESS' section for more details)
Usage:
<list_code_definition_names>
<path>Directory path here</path>
...
const trinityPromptTemplate = `You are Cline, a highly skilled software engineer with extensive knowledge in many programming languages, frameworks, design patterns, and best practices.
...
function renderPromptTemplate(
  ...
  const detectedCliTools =
    runtimeInfo.detectedCliTools.join(", ") || "(none detected)";

  return template
    .replaceAll("{{cwd}}", runtimeInfo.currentWorkingDirectory)
    .replaceAll("{{operatingSystem}}", runtimeInfo.operatingSystem)
    .replaceAll("{{defaultShell}}", runtimeInfo.defaultShell)
    .replaceAll("{{homeDirectory}}", runtimeInfo.homeDirectory)
    .replaceAll("{{detectedCliTools}}", detectedCliTools);
}

export function renderClineSystemPromptTemplate(
  family: ClinePromptFamily,
  runtimeInfo: PromptRuntimeInfo,
) {
  return renderPromptTemplate(
    family === "trinity" ? trinityPromptTemplate : genericPromptTemplate,
    runtimeInfo,
  );
}
